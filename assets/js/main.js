...
const wordsList = (str, subStr) => {
  ...
  str = str.replace(/[^a-z\s]/gi, ''); // удаляем лишние знаки
  console.log(str);
  console.log('-------------------------------------------');
  ...
  let arr = str.split(' '); // из строки в массив, для разделения space
  console.log(arr);
  console.log('-------------------------------------------');

  arr = arr.filter(function (funk) { return funk.indexOf(subStr) > -1 }); // фильтруем значения в массиве
  console.log(arr);

  console.log('-------------------------------------------');

  let set = new Set(); // создаем коллекцию

  arr.forEach((arrItem) => {   // записывам в коллекцию все значения
    set.add(arrItem);
  });
...
function getDateAgo(date, days) {
  ...
  return day.toLocaleString().replace(/(\d.*),\s+(\d.*)/gu, '$1');
}
